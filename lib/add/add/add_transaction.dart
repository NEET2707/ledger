...
class _AddTransactionState extends State<AddTransaction> {
  ...
  int? _isCredit;
  // final TextEditingController amtcon = TextEditingController();
  ...
  Future<void> _fetchTransactionDetails(int id) async {
    ...
    if (result.isNotEmpty) {
      ...
      setState(() {
        ...
        _transactionDate = DateFormat('yyyy-MM-dd')
            .parse(transaction[textlink.transactionDate] as String);

        final reminderRaw = transaction[textlink.transactionReminderDate] as String?;
        if (reminderRaw != null && reminderRaw.isNotEmpty) {
          try {
            _reminderDate = DateFormat('yyyy-MM-dd').parse(reminderRaw);
        ...
        print("55555555555555555555555555555555555555555555555555555555$name");
        noteController.text = transaction[textlink.transactionNote]?.toString() ?? '';
      });

      print("Fetched transaction details: $transaction");
    }
  }
  ...
  Future<void> _selectDate(BuildContext context, DateTime initialDate,
    ...
    final safeInitialDate =
        initialDate.isBefore(DateTime(2000)) ? DateTime.now() : initialDate;
                ...
                InkWell(
                  onTap: () async {
                    ...
                    if (result != null) {
                      setState(() {
                        accountId =
                            int.tryParse(result["accid"]?.toString() ?? '0') ??
                                0; // Safely parse the id
                        name = result["accname"]?.toString() ??
                        ...
                        onPressed: () {
                          if (_formKey.currentState?.validate() ?? false) {
                            final transactionData = {
                              textlink.transactionAmount:
                                  double.parse(amtcon.text),
                              ...
                              textlink.transactionReminderDate:
                                  _reminderDate != null
                                      ? _formatDate(_reminderDate!)
                                      : null,
                              textlink.transactionNote:
                                  _isCredit == 1 ? 'Credit Note' : 'Debit Note',
                            ...
                            final transactionData = {
                              ...
                              textlink.transactionReminderDate:
                                  _reminderDate != null
                                      ? _formatDate(_reminderDate!)
                                      : null,
                              textlink.transactionNote: widget.iscredit == true
                                  ? 'Debit Note'
                                  : 'Credit Note',
                              textlink.transactionIsCredited: _isCredit ?? 0,
                              transaction_accountId: accountId,
